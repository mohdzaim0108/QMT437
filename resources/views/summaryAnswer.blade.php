                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h1 class="h3 m-0 font-weight-bold text-primary">Summary Answering Question</h1>
                        </div>
                        <div class="card-body">
                            <div class="container">
                                <h1>Summary answer : {{ $question->questionTitle }}</h1><br>
                                <h3>Question : {{ $question->question }}</h3><br>
                                <hr>
                                @foreach ($questionSteps as $step)
                                    <h3>Step {{ $loop->index + 1 }}: {{ $step->instruction }}</h3>
                                    <p>Answer scheme: {{ $step->answer }}</p>
                                    <p>Your Answer: {{ $userAnswers[$step->questionStepId] ?? 'No answer given' }}</p>
                                    <hr>
                                @endforeach
                                <div class="text-center">
                                    <button type="button" class="btn btn-primary" onclick="window.location.href = '{{ route('listQuestionS') }}'">Back to Questions List</button>
                                </div>
                            </div>
                        </div>
                    </div>
